...
intro :: forall t m. (MonadWidget t m, RouteToUrl (R Route) m, SetRoute t (R Route) m) => Section t m
  ...
  segments def $ for_ (progressTable @t @m) $ \(Category name items) -> do
    ...
    table (def & tableConfig_attached |?~ Attached & action ?~ actionConfig) $ do
      ...
      tbody $ for_ items $ \(item, status, mWidget) -> tr $ do
        td $ case mWidget of
          Nothing -> text item
          Just _ -> routeLink (Route_Section :/ toId item) $ text item
        case status of
...
routeEncoder = isoEncoder $ iso
  (\case
    Route_Main :/ () -> mempty
    Route_Section :/ s -> ([], M.singleton "section" $ Just s))
  (\pn -> case join . M.lookup "section" $ snd pn of
    Nothing -> Route_Main :/ ()
    Just s -> Route_Section :/ s)
...
runWithLoader :: (DomBuilderSpace m ~ GhcjsDomSpace, ObeliskWidget t x (R Route) m, RouteToUrl (R Route) m) => m ()
runWithLoader = do
...
loadingDimmer evt =
  dimmer (def & dimmerConfig_page .~ True & action ?~ def
    { _action_transition = Transition Fade Nothing def <$ evt }) $
    divClass "ui huge text loader" $ text "Loading semantic-reflex docs..."

app :: forall t m x. (DomBuilderSpace m ~ GhcjsDomSpace, ObeliskWidget t x (R Route) m, RouteToUrl (R Route) m) => m ()
app = do
