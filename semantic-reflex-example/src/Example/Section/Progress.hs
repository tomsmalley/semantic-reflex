...
randomRangeControl :: (MonadWidget t m, Prerender t m) => (Int, Int) -> Event t Int -> m (Event t (Int -> Int))
...
progressSection :: forall t m. (MonadWidget t m, Prerender t m) => Section t m
