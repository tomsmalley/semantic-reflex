...
data LabelConfig t = LabelConfig
  ...
  , _detail :: RenderWhen t (PlainText t)
  , _link :: Bool
...
instance (t ~ t') => UI t' m Inline (Label t) where
  type Return t' m (Label t) = LabelResult t m
  ui' = unRestrict . ui'

instance (t ~ t') => UI t' m None (Label t) where
  type Return t' m (Label t) = LabelResult t m

  ui' (Label txt config@LabelConfig {..}) = do
    (e, (imageResult, leftIconResult, rightIconResult)) <-
      reRestrict $ elWithAnim' elType attrs $ do
        -- FIXME needs to remove ui class
        imageResult <- runRenderWhen ui' _image
        leftIconResult <- runRenderWhen ui' _leftIcon
        activeText txt
        rightIconResult <- runRenderWhen ui' _rightIcon
        runRenderWhen (divClass "detail" . ui') _detail
